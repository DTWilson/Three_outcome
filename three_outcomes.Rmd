...
Plot of inconclusive region as a function of sample size:
```{r}
get_limits <- function(n, c, delta)
{
  ps <- seq(0, 1, 0.0001)
  qs <- 1 - pnorm(c, ps, sqrt(ps*(1-ps)/n))
  c(ps[tail(which(qs <= delta), 1)], 
    ps[head(which(qs >= (1 - delta)), 1)])
}

df <- data.frame(n = 5:100)
df <- cbind(df, t(sapply(df$n, get_limits, c = 0.8, delta = 0.05)))
names(df)[2:3] <- c("l", "u")

ggplot(df, aes(n)) + geom_line(aes(y = l)) + geom_line(aes(y = u)) + coord_flip() +
  ylim(c(0,1))
```
...
For each optimal design, find and plot the opterating characteristics if in fact $\eta = 0.5$:
...
#ggsave("./paper/figures/eta_all.pdf", height=9, width=14, units="cm")
#ggsave("./paper/figures/eta_all.eps", height=9, width=14, units="cm", device = cairo_ps())
...
For an example design using Storer's method, plot the probability of each desicion as a function of the unknown parameter.

```{r}
n <- 115; x0 <- 64; x1 <- 73
rhos <- seq(0, 1, l = 500)

df <- data.frame(rho = rep(rhos, 3))
df$pr <- c(pbinom(x0, n, rhos), 
           pbinom(x1, n, rhos) - pbinom(x0, n, rhos),
           1 - pbinom(x1, n, rhos))
df$t <- rep(c("a", "b", "c"), each = 500)

ggplot(df, aes(rho, pr, colour = t, linetype = t)) + geom_line() +
  scale_colour_manual(name = "", 
                      labels = c("Stop", "Intermediate", "Go"), 
  ...
  xlab(expression(paste("Probability of adherance, ", rho))) +
  ylab("Probability of decision") +
  theme_minimal()

#ggsave("./paper/figures/storer_ex.pdf", height=9, width=14, units="cm")
#ggsave("./paper/figures/storer_ex.eps", height=9, width=14, units="cm", device = cairo_ps())
```
...
Allowing adjustments with known effect $tau$.
...
ggplot(df, aes(tau)) + geom_line(aes(y=n)) +
  geom_line(aes(y = x0), colour=cols[3]) + geom_line(aes(y = x1), colour=cols[4]) +
  #geom_line(aes(y = dif), linetype=2) + 
...
Now, take the optimal design when we assumed $\tau = 0.1$ and find its actual OCs for a range of actual $\tau$ observed after the pilot.
...
#ggsave("./paper/figures/tau_all.pdf", height=9, width=14, units="cm")
#ggsave("./paper/figures/tau_all.eps", height=9, width=14, units="cm", device = cairo_ps())
```
